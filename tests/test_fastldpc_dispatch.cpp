...
extern "C" unsigned fastldpc_test_x86_features_c () { return cpuFeatures; }
...
extern "C" void fastldpc_simd_set_ft8_mode_c (int mode)
{
    ++modeCalls;
    lastMode = mode;
}
...
extern "C" void fastldpc_simd_decode174_91_batch_c (
    ...
    signed char*, int*, int*, float*) { ++simdCalls; }
extern "C" int fastldpc_simd_extrinsic174_91_c (float const*, int, float, float* out)
{
    ++extrinsicCalls;
    out[0] = 0.25f;
    return 1;
}
extern "C" void fastldpc_simd_gate_dump_open_c (char const*) { ++gateCalls; }
extern "C" void fastldpc_simd_gate_dump_close_c () { ++gateCalls; }
extern "C" void fastldpc_simd_gate_truth_set_c (signed char const*) { ++gateCalls; }
extern "C" void fastldpc_simd_gate_truth_clear_c () { ++gateCalls; }

extern "C" void fastldpc_set_enabled_c (int);
extern "C" int fastldpc_is_enabled_c ();
extern "C" void fastldpc_set_ft8_mode_c (int);
extern "C" int fastldpc_extrinsic174_91_c (float const*, int, float, float*);
extern "C" void fastldpc_gate_dump_open_c (char const*);
extern "C" void fastldpc_gate_dump_close_c ();
extern "C" void fastldpc_gate_truth_set_c (signed char const*);
...
int main (int argc, char** argv)
{
    ...
    if (scenario == "ivybridge") cpuFeatures &= ~(2u | 4u); // AVX, no AVX2/FMA
        ...
        return 1;
#if defined(_WIN32)
    _putenv_s ("DECODIUM_FT2_DISABLE_FASTLDPC", emergency ? "1" : "0");
#else
    setenv ("DECODIUM_FT2_DISABLE_FASTLDPC", emergency ? "1" : "0", 1);
#endif

    // An enabled saved setting must not bypass CPU/OS or emergency guards.
    fastldpc_set_enabled_c (scenario != "ui-disabled");
    bool const expectedSimd = scenario == "supported";
    if (fastldpc_is_enabled_c () != int (expectedSimd)) {
    ...
    int const accepted = fastldpc_extrinsic174_91_c (llr.data (), 2, 2.0f, extrinsic.data ());
    ...
    fastldpc_set_enabled_c (0);
    extrinsic.fill (99.0f);
    if (fastldpc_extrinsic174_91_c (llr.data (), 2, 2.0f, extrinsic.data ()) != 0
        || !std::all_of (extrinsic.begin (), extrinsic.end (), [] (float v) { return v == 0; })
        || extrinsicCalls != int (expectedSimd)) return 1;
    fastldpc_extrinsic174_91_c (nullptr, 2, 2.0f, extrinsic.data ());
    fastldpc_extrinsic174_91_c (llr.data (), 2, 2.0f, nullptr);
    ...
    fastldpc_gate_dump_open_c ("unused-test-path");
    fastldpc_gate_truth_set_c (codeword.data ());
