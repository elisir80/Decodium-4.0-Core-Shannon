...
| 26 | Hostile WAV/image/metadata/network input is bounded and crash-safe | local-pass | Strict parsers, allocation/path/symlink controls, duplicate-key rejection, deterministic fuzz smoke and focused ASan/UBSan runs cover the main boundaries. Final libFuzzer corpus runs and a complete current-tree sanitizer run remain open. |
