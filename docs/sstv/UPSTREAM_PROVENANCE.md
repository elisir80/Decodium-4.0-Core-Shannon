...
| `src/sstv/diagnostics/*` | Existing Decodium/Qt logging and atomic-file facilities; no upstream SSTV code | Original strict allowlist, bounded 512-event ring and version-1 diagnostic exporter. The final Bridge projection uses stable ASCII source tokens rather than translated/slash-delimited labels, refreshes active TX at no more than 4 Hz plus terminal state, represents unavailable HAMDRM explicitly rather than fabricating zero metrics, and preserves the last terminal test-tone result. The test-tone action is guarded and labelled as real PTT/RF transmission. Images, audio, paths, URLs, credentials and personal text remain rejected. Rebuilt controller/QML coverage is included in the final 81/81 SSTV pass. |
